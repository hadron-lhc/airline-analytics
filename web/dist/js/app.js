...
(() => {
    ...
    const queueMd = (rows) => {
      ...
      rows.forEach((q) => {
        out += `| ${escMd(q.airport)} | ${(q.processed || 0).toLocaleString()} | ${Math.round(q.wait_avg_s || 0)}s | ` +
          `${Math.round(q.wait_p90_s || 0)}s | ${Math.round(q.wait_max_s || 0)}s | ` +
          `${pctInt(q.congested_pct)} | ${q.peak_hour ? q.peak_hour + "h" : "—"} |\n`;
      });
      return out;
    };
  ...
  function heatmapMd(hm) {
    ...
    airports.forEach((code) => {
      const byHour = {};
      (hm[code] || []).forEach(([h, v]) => { byHour[h] = v; });
      out += `| ${escMd(code)}` + hours.map((h) => ` | ${byHour[h] || 0}`).join("") + " |\n";
    });
    return out;
  }
  ...
  function downloadReportHtml() {
    const r = report || buildClientReport();
    if (!r) return;
    const body = reportSections(r, !!report);
    const metaInfo = r.meta || {};
    const html = buildReportHtml(r, body, metaInfo);
  ...
  function downloadReportMarkdown() {
    const r = report || buildClientReport();
    const md = r && getReportMarkdown(r);
    if (!md) return;
  ...
  function downloadReportPdf() {
    ...
    if (!win) return;
    win.document.open();
    win.document.write(buildReportHtml(r, body, metaInfo));
    win.document.close();
    win.focus();
...
<style>
  ...
  *{box-sizing:border-box}
  body{font-family:Inter,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;margin:0;background:var(--bg);color:var(--text);padding:32px 24px}
  .wrap{max-width:960px;margin:0 auto;background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:28px}
  h1{font-size:22px;margin:0 0 4px}
  .report-sub{color:var(--muted);font-size:12px;margin:0 0 20px}
  h3{font-size:14px;margin:24px 0 10px;color:var(--text);border-bottom:1px solid var(--border);padding-bottom:6px}
  h4{font-size:12px;margin:14px 0 6px;color:var(--muted);font-weight:600}
  .report-counters{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:20px}
  .counter{flex:1 1 140px;background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:12px 14px;text-align:center}
  .counter .num{font-size:28px;font-weight:800}
  .counter .lbl{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.4px}
  table.report-table{width:100%;border-collapse:collapse;font-size:12px}
  .report-table th{text-align:left;color:var(--muted);padding:6px 8px;border-bottom:1px solid var(--border);white-space:nowrap}
  .report-table td{padding:6px 8px;border-bottom:1px solid var(--border);white-space:nowrap}
  .report-table-wrap{overflow-x:auto}
  .pill{display:inline-block;border-radius:10px;font-size:11px;font-weight:600;padding:2px 8px}
  .pill.ok{background:rgba(22,163,74,.14);color:var(--good)}
  ...
  .report-kpis>div{background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px 12px}
  .report-kpis b{font-size:18px;display:block}
  .report-kpis span{font-size:11px;color:var(--muted)}
  .report-badge{display:none}
  .hm-grid{display:grid;grid-template-columns:64px repeat(auto-fit,minmax(40px,1fr));gap:2px;margin-bottom:2px}
  .hm-head{font-size:10px;color:var(--muted);text-align:center;padding-top:2px}
  .hm-apt{font-size:11px;color:var(--muted);font-weight:600;padding-top:6px}
  .hm-cell{border-radius:4px;background:color-mix(in srgb,#2563eb calc(var(--hm,0)*100%),transparent);color:#fff;font-size:10px;text-align:center;padding:5px 0;min-width:34px}
  @media print{body{padding:0;background:#fff}.wrap{border:none;box-shadow:none;max-width:none}}
</style>
</head>
<body>
<div class="wrap">
  ...
  function wireReportDownload() {
    $("btn-download-md").addEventListener("click", downloadReportMarkdown);
    $("btn-download-html").addEventListener("click", downloadReportHtml);
    $("btn-download-pdf").addEventListener("click", downloadReportPdf);
  }
